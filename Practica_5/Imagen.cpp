 ...
 */
Imagen::Imagen(const std::string &_id, const std::string &_nombre, int _tam,
               const Fecha &_fecha):
               id(_id) , nombre(_nombre) , tam(_tam) , fecha(_fecha){
    etiquetada=deque<Etiqueta*>();
};

/**
 * Constructor de copia
 */
Imagen::Imagen(const Imagen &orig):id(orig.id), nombre(orig.nombre), tam(orig.tam), fecha(orig.fecha), etiquetada(orig.etiquetada) {}
 ...
 */
vector<string> Imagen::getEtiquetas() {
    vector<string> salida;
    for(int i = 0; i < etiquetada.size(); i++){
        salida.push_back(etiquetada[i]->getNombre());
    }
    return salida;
}
 ...
 * @param origen [in] imagen que se va a asignar
 */
...
void Imagen::mostrar() {
    ...
    std::cout << " ID=" << getId()
              << " Fichero=" << getNombre() << " Tam=" << getTam()
              << " Fecha=" << fecha
              << " Etiquetas=";
    for(int i = 0; i < etiquetada.size(); i++){
        std::cout << etiquetada[i]->getNombre() << " ";
    }
    std::cout << std::endl;
}
...
/**
 * @brief Añade una etiqueta a la imagen
 * @param etiqueta [in] etiqueta que se quiere añadir
 */
void Imagen::anadirEtiqueta(Etiqueta *etiqueta) {
    if (etiqueta)
        etiquetada.push_back(etiqueta);
    else throw std::invalid_argument("Imagen::anadirEtiqueta: La etiqueta es un puntero nulo");
}